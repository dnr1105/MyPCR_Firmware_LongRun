...
UINT32 temp_time_1 = 0;
UINT32 temp_time_2 = 0;
UINT32 temp_time_3 = 0;
UINT32 temp_time_4 = 0;
...
//    memcpy(&temp_time_1, Rx_Buffer[RX_TIME_1], sizeof(unsigned long));
//    memcpy(&temp_time_2, Rx_Buffer[RX_TIME_2], sizeof(unsigned long));
//    memcpy(&temp_time_3, Rx_Buffer[RX_TIME_3], sizeof(unsigned long));
//    memcpy(&temp_time_4, Rx_Buffer[RX_TIME_4], sizeof(unsigned long));

    temp_time_1 = ((UINT32)Rx_Buffer[RX_TIME_1] * 16777216);
    temp_time_2 = ((UINT32)Rx_Buffer[RX_TIME_2]*65536);
    temp_time_3 = ((UINT32)Rx_Buffer[RX_TIME_3]*256);
    temp_time_4 = ((UINT32)Rx_Buffer[RX_TIME_4]);
    Task_Time = (temp_time_1) + (temp_time_2) + (temp_time_3) + temp_time_4;

    //    Task_Time = (UINT32)(Rx_Buffer[RX_TIME_1] << 24);
    //    Task_Time += (UINT32)(Rx_Buffer[RX_TIME_2] << 16);
    //    Task_Time += (UINT32)(Rx_Buffer[RX_TIME_3] << 8);
    //    Task_Time += (UINT32)(Rx_Buffer[RX_TIME_4]);


    //    Task_Time = ((UINT32)Rx_Buffer[RX_TIME_1] * 16777216);
    //    Task_Time += ((UINT32)Rx_Buffer[RX_TIME_2] * 65536);
    //    Task_Time += ((UINT32)Rx_Buffer[RX_TIME_3] * 256);
    //    Task_Time += ((UINT32)Rx_Buffer[RX_TIME_4]);

    //    Task_Time = 32769;
//    sendCopyBuffer();

    //    Task_Time = (Rx_Buffer[RX_TIME_1]*16777216)
    //            + (Rx_Buffer[RX_TIME_2]*65536)
    //            +(Rx_Buffer[RX_TIME_3]*256)
    //            +(Rx_Buffer[RX_TIME_4]);

    //    Task_Time = (Rx_Buffer[RX_TIME_1]<<24)
    //        +(Rx_Buffer[RX_TIME_2]<<16)
    //        +(Rx_Buffer[RX_TIME_3]<<8)
    //        +(Rx_Buffer[RX_TIME_4]);
...
void Calc_Time(void) {
    ...
    for (i = 0; i < Total_ActLine; i++) {
        ...
        if (PCR_Task_Line[i].Label != AF_GOTO) // Add Time
            Left_Time += PCR_Task_Line[i].Time;
        if (PCR_Task_Line[i].Label == AF_GOTO) // Goto Line Processing
        {
            // YJ
            //	Cur_Loop_Count = PCR_Task_Line[i].Temp;	
            while (TRUE) {
                UINT32 temp = (UINT32) PCR_Task_Line[i].Temp;
                if ((UINT32) PCR_Task_Line[line++].Label == temp)
                    break;
            }
            ...
            for (j = 0; j < PCR_Task_Line[i].Time; j++) {
                for (k = line - 1; k < i; k++)
                    Left_Time += PCR_Task_Line[k].Time;
            }
        }
    }
}
...
void Task_Control(void) {
    ...
    // Left time per line
    Left_Time_Q = (Left_SecTime >> 16) & 0xff;
    Left_Time_H = (Left_SecTime >> 8) & 0xff;
    Left_Time_L = Left_SecTime;

    //    Left_Time_Q = (BYTE)(Left_SecTime/65536);
    //	Left_Time_H = (BYTE)(Left_SecTime/256);
    //	Left_Time_L = (BYTE)Left_SecTime;

    // Total lef time
    Left_TotalTime_Q = (Left_Time >> 16) & 0xff;
    ToSendDataBuffer[TX_KI_1] = (Left_Time >> 16) & 0xff;
    Left_TotalTime_H = (Left_Time >> 8) & 0xff;
    Left_TotalTime_L = (Left_Time);

    //    Left_TotalTime_Q = (BYTE)(Left_Time/65536);
    //	Left_TotalTime_H = (BYTE)(Left_Time/256);
    ...
    if (Left_SecTime == 0 && !IsTimeInfinite) // if SecLeft = 0, Load the next action
    {
        ...
        if (PCR_Task_Line[Cur_Action_Number].Label != AF_GOTO) // if the action is not GOTO
        {
            ...
            // Left time (sec)
            Left_SecTime = PCR_Task_Line[Cur_Action_Number].Time;
        ...
        } else // if the action is GOTO
        {
            ...
            if (Loop_Count < 0)
                Loop_Count = (PCR_Task_Line[Cur_Action_Number].Time);
...
void Post_Process(void) {
    ...
    //   	ToSendDataBuffer[TX_TIMEH] = -100;
    //	ToSendDataBuffer[TX_TIMEL] = -100;

    //    ToSendDataBuffer[TX_TIME_1] = ((Task_Time & 0xFF000000) >> 24) & 0xff;
    //    ToSendDataBuffer[TX_TIME_2] = ((Task_Time & 0x00FF0000) >> 16) & 0xff;
    //    ToSendDataBuffer[TX_TIME_3] = ((Task_Time & 0x0000FF00) >> 8) & 0xff;
    //    ToSendDataBuffer[TX_TIME_4] = ((Task_Time & 0x000000FF)) & 0xff;
    ToSendDataBuffer[TX_TIME_1] = (int) ((Task_Time >> 24) & 0xff);
    ToSendDataBuffer[TX_TIME_2] = (int) ((Task_Time >> 16) & 0xff);
    ToSendDataBuffer[TX_TIME_3] = (int) ((Task_Time >> 8) & 0xff);
    ToSendDataBuffer[TX_TIME_4] = (int) ((Task_Time) & 0xff);
...
void sendCopyBuffer(void) {
    ToSendDataBuffer[TX_KP] = (int) ((Task_Time >> 16) & 0xff);
    ToSendDataBuffer[TX_KI] = (int) ((Task_Time >> 8) & 0xff);
    ToSendDataBuffer[TX_KD] = (int) ((Task_Time) & 0xff);
    ToSendDataBuffer[TX_KP_1] = Rx_Buffer[RX_TIME_2];
    ToSendDataBuffer[TX_KI_1] = Rx_Buffer[RX_TIME_3];
    ToSendDataBuffer[TX_KD_1] = Rx_Buffer[RX_TIME_4];
}
